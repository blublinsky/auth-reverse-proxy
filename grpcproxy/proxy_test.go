...
var testBackend = flag.String("test-backend", "", "Service providing PingServiceServer")
...
func TestLegacyBehaviour(t *testing.T) {
	...
	proxyClient := grpcserver.NewPingServiceClient(proxyCC)

	// 4. Run the tests!
	grpcserver.TestPingServiceServerImpl(t, proxyClient)
}
...
func TestNewProxy(t *testing.T) {
	...
	go func() {
		...
		// First, we need to create a client connection to this backend.
		proxySrv, h := grpcproxy.NewProxy(testCC)
...
func backendDialer(t *testing.T, opts ...grpc.DialOption) (*grpc.ClientConn, error) {
	...
	grpcserver.RegisterPingServiceServer(testSrv, grpcserver.DefaultPingServiceServer)
